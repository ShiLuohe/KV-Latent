...
def get_model(mla_config: MLAConfig, load_model=False, return_original_model=False):
    tokenizer = PreTrainedTokenizerFast.from_pretrained("models/Meta-Llama-3-8B")
    tokenizer.pad_token = tokenizer.eos_token
    model = LlamaForCausalLM.from_pretrained("models/Meta-Llama-3-8B").half()
    ...
    if return_original_model:
        new_model = LlamaForCausalLM.from_pretrained("models/Meta-Llama-3-8B").half()
