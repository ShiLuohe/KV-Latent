...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, orig_model: LlamaModel, config):
        super().__init__(config)
        self.padding_idx = orig_model.padding_idx
        self.vocab_size = orig_model.vocab_size

        self.embed_tokens = orig_model.embed_tokens
        self.layers = nn.ModuleList([LlamaDecoderLayerWithMLA(decoderlayer, 2, 2, 256, 512) for decoderlayer in orig_model.layers])
    ...
    def forward(
        ...
        if use_cache:
            selected_position_ids = torch.arange(seq_length_with_past, device=position_ids.device).unsqueeze(0).expand(batch_size, -1)
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if use_cache:
                if past_key_value is None:
                    infer_attention_mask = attention_mask[:, :, -hidden_states.shape[1]:, -hidden_states.shape[1]:]
                else:
                    if (past_key_value[0].shape[-2] + 1) > attention_mask.shape[-1]:
                        pass
                    infer_attention_mask = attention_mask[:, :, -(past_key_value[0].shape[-2] + 1):, -(past_key_value[0].shape[-2] + 1):]
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = decoder_layer(
                    hidden_states,
                    attention_mask=infer_attention_mask if use_cache else attention_mask,
                    position_ids=position_ids,
                    past_key_value=past_key_value,
                    output_attentions=True if use_cache else output_attentions,
                    use_cache=use_cache,
                    selected_position_ids=selected_position_ids if past_key_values is None and use_cache else None,
                )
...
class LlamaForCausalLMWithMLA(LlamaPreTrainedModel):
    ...
    def __init__(self, original_model: LlamaForCausalLM, MLAconfig: MLAConfig):
        ...
        self.model = LlamaModel(original_model.model, original_model.config)
